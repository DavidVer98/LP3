...
int main()
{
   ...
   fgets(caracteres, 600, archivo);
   //printf("%s",caracteres);
   size_t st = strlen(caracteres);
   int a = write_temp_file(caracteres, st);
   char *tf = read_temp_file(a, &st);
   printf("Texto del archivo temporal: %s\n", tf);
   fclose(archivo);
   return 0;
}